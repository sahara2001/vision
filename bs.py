...
def getHand(img, ub, lb,prev=None,motion=None):
...
def getFingerTip(x,depth,motion=None):
...
def getFingerTip1(x,depth,prev=None,motion=None):
    #depth = cv2.GaussianBlur(depth,(5,5),0)
...
def getBackground(imgs):
    # return mean and std per image per channel
    return np.mean(imgs,axis=0), np.std(imgs,axis=0)

def backgroundSubtraction(img, bg, std):
    # get pixels above noise level
    rs = (np.abs(img - bg)/std) > NOISE
    return rs
